...
>>>>>>> cba9289fa7670dcb898557df6cfbcb0e565526d4
...
The “upchain” (wind speed increasing monotonically) converged in about 2000-3000 steps but the "downchain" did not seems to converge. This may due to a bad prior.

## Plots for Gibbs sampling with Intercept
...
![Histogram of Betas](./figures/hist_beta.jpg)

![Trace of Betas](./figures/trace_beta.jpg)
...
In this project, we constructed a hierarchical Bayesian model to predict the wind speeds of hurricanes. Because we were able to derive the conditional distributions we achieved this model by implementing a gibbs sampling algorithm for 5000 iterations. Originally we implemented this algorithm with 7 predictors and the intercept. All predictors had converged within these 5000 iterations, however the intercept value did not. We then implemented the same algorithm without the intercept and we had all predictors converge again within the 5000 iterations. We compared the results of the models with and without the intercepts. The results showed that our models gave us a set of adequate predicted values. 
...
We ran 5000 iterations to obtain the model, however, we might need to construct the full model with more iterations. We set the number to be 5000 because we want to check the performance of the chain efficiently and see if all the variables converge. Also, more combinations of initial values should be tested to achieve optimal performance. Further research should be done on why the intercept is not converging.
