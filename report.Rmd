...
summaryplotsfun <- function(input_chain){
  ...
  p1 <- betasummary %>% 
    ...
    labs(title = "Figure.1 Estimated Coefficients for Variables (n.iter = 1000)",
  ...
  p2 <- betasummary %>% 
    ...
    geom_line() +
    facet_wrap(~var, nrow = 2, scales = "free") +
    labs(title = "Figure.1 Markov Chain of Estimated Coefficients for Variables (n.iter = 1000)",
         x = "Iteration Index", y = "Values")
...
### Further Discussion on MC chain:
...
We can see that, the intercept still diverged, and appeared as a two-peak normal distribution. This suggested that some other parts in the model itself is questionable.

2. Investigate the models root mean square error:

Though the intercept do not converge, we can use other methods to see whether the whole model converged. To do so, we would reconfirm with SSR. If the SSR converged after some steps, it may be proper to say that our chain converged. 
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=5}

test3 = test <- mcmc(dat, 
             ini.beta = c(50,rep(0,7)), 
             ini.sig = .5, 
             ini.bsig = diag(.5,8,8), niter = 20000)
ssri = cbind(c(1:20000), test$ssr) %>% as.data.frame()
plot1 = ggplot(aes(x = V1, y = V2), data = ssri) +
  geom_line() +
  labs(title = "Markov Chain of SSR values", 
       x = "Iteration Index",
       y = "SSR") +
  theme_bw()

plot2 = ggplot(aes(x = V2), data = ssri) +
  geom_histogram() +
  labs(title = "Markov Chain of SSR values", 
       x = "SSR",
       y = "Frequency") +
